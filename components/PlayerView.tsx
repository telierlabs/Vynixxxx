        ...
        <div className="relative aspect-video w-full bg-zinc-900 rounded-xl overflow-hidden mb-4 border border-white/5">
          <div className="absolute inset-0 flex items-center justify-center">
             <div className="text-center">
                ...
                <p className="text-[10px] uppercase tracking-[0.3em] text-zinc-500 font-black">Buffering Vynix Vision...</p>
             </div>
          </div>
          {/* Controls Overlay (Simple) */}
          <button 
            onClick={onBack}
            className="absolute top-4 left-4 p-2 bg-black/40 backdrop-blur-xl rounded-full hover:bg-white hover:text-black transition-all"
          >
            <ChevronLeft size={20} />
          </button>
        </div>
        ...
        {relatedMovies.map((m) => (
          ...
          >
            <div className="relative w-36 aspect-video shrink-0 bg-zinc-900 rounded-lg overflow-hidden border border-white/5">
              <img src={m.imageUrl} alt={m.title} className="w-full h-full object-cover noir-image group-hover:scale-105 transition-transform" />
              <span className="absolute bottom-1 right-1 bg-black/80 px-1 rounded text-[8px] font-bold">2:14:00</span>
            </div>
